...
class DataBase:
    ...
    def create_table_execution(self):
        cursorObj = self.con.cursor()
        cursorObj.execute('DROP table if exists executions')
        cursorObj.execute("CREATE TABLE executions(id INTEGER PRIMARY KEY AUTOINCREMENT,name text, number_layers integer,number_params integer)")
        self.con.commit()     
    def insert_execution(self,values):
        cursorObj = self.con.cursor()
        cursorObj.execute('INSERT INTO executions(name,number_layers,number_params) VALUES(?,?,?)', values)
        self.con.commit()
