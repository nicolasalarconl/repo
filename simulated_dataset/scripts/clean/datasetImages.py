...
import numpy as np
import time
from auxiliar.read import read_pkl 


# %%
class DatasetImages:  
    
    ## TODO CAMBIAR SIZE_IMAGE = SIZE_FIGURE
    ...
    def save(self,size_image,params,stop,start = None,path = None):
        ...
        for index in cp.arange(int(start),int(stop),1):
            ...
            hdu_image.writeto(self.path_save+'/image_'+str(self.size_image)+'x'+str(self.size_image)+'_'+str(index)+'.fits',
                            overwrite=True)
            AuxiliaryFunctions.write_txt(
                     self.path_save+'/image_'+str(self.size_image)+'x'+str(self.size_image)+'_'+str(index)+'.txt',
                cleanImage.mask)
            stop_time = time.time()
